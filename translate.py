...
def translate_sequence(rna_sequence, genetic_code):
    ...
    if len(rna_seq_upper) < 3:
        return ""   
    elif genetic_code[rna_seq_upper[0:3]] == '*':
        return ""
    else:
        x = range(0, len(rna_seq_upper), 3)
        codon_list = []
        amino_seq_list = []
        for element in x:
            if len(rna_seq_upper[element:element+3]) < 3:
                break
            codon_list.append(rna_seq_upper[element:element+3])
        for item in codon_list:
            if genetic_code[item] == '*':
                break
            else:
                amino_seq_list.append(genetic_code[item])
...
def get_longest_peptide(rna_sequence, genetic_code):
    ...
    amino_seq = get_all_translations(rna_sequence, genetic_code)
    rev_comp_amino_seq = get_all_translations(rev_comp_seq, genetic_code)
    all_amino_seq = amino_seq + rev_comp_amino_seq
    if all_amino_seq:
        longest_peptide = max(all_amino_seq, key=len)
